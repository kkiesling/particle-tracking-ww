...
# python generate-wwig.py RATIO .vtk .h5m
...
def get_data(fvtk, e_bounds):
    ...
    for i in range(len(e_bounds)):
        ...
        mindata = min(mf.cell_data['hexahedron'][dataname]
                      [mf.cell_data['hexahedron'][dataname] > 0])
...
def generate_wwigs(wwig_info, fvtk, ratio, id_rng):
    ...
    for ID, info in wwig_info.items():
        ...
        if int(ID) in id_rng:
            ...
            driver.create_geometry(ig, tag_for_viz=True, tags=tags, sname=sname)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4:
        id_rng = [int(x) for x in sys.argv[4:]]
    else:
        id_rng = range(len(e_bounds))

    wwig_info = get_data(fvtk, e_bounds)

    generate_wwigs(wwig_info, fvtk, ratio, id_rng)
